...
@section('content')
    <div class="page-heading">
        @include('admin.partials.page-header', [
            ...
            'subtitle' => 'Pantau pengajuan peminjaman dan pengembalian dari pegawai.',
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-3">
                    <div>
                        ...
                        <p class="mb-0 text-muted">Semua notifikasi untuk akun admin ditampilkan di sini.</p>
                    </div>
                    <div class="d-flex align-items-center flex-wrap gap-2">
                        ...
                        @if ($unreadNotificationCount > 0)
                            <form method="POST" action="{{ route('admin.notifications.read-all') }}" data-swal-confirm data-swal-icon="question" data-swal-title="Tandai semua dibaca?" data-swal-text="Semua notifikasi admin akan ditandai sudah dibaca." data-swal-confirm-text="Ya, tandai" data-swal-confirm-color="#435ebe">
                                @csrf
                                @method('PATCH')
                ...
                <div class="card-body">
                        ...
                        <div class="notification-list-item border rounded-3 p-3 {{ $loop->last ? '' : 'mb-3' }} {{ $isUnread ? 'bg-light' : 'bg-white' }}">
                            <div class="d-flex align-items-start gap-3 flex-wrap flex-md-nowrap">
                                ...
                                <div class="flex-grow-1">
                                    <div class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-2">
                                        <div>
                                            ...
                                            <small class="text-muted">
                                                ...
                                                <span class="mx-1">&middot;</span>{{ $notification->created_at->diffForHumans() }}
                                            </small>
                                        </div>
                                        <a href="{{ route('admin.notifications.show', $notification) }}" class="btn btn-light-primary btn-sm icon icon-left">
                                    ...
                                    @if (!empty($meta))
                                        <div class="d-flex align-items-center flex-wrap gap-2">
                                            @if (!empty($meta['employee_name']))
                                                <span class="badge bg-light-secondary">{{ $meta['employee_name'] }}</span>
                                            @endif
                                            ...
                                            @if (!empty($meta['planned_return_date']))
                                                <span class="badge bg-light-warning">Rencana kembali {{ $meta['planned_return_date'] }}</span>
                                            @endif
                                            @if (!empty($meta['report_number']))
                                                <span class="badge bg-light-info">{{ $meta['report_number'] }}</span>
                                            @endif
                                            @if (!empty($meta['condition']))
                                                <span class="badge bg-light-success">{{ $meta['condition'] }}</span>
                                            @endif
                                        </div>
                                    @endif
                                </div>
                    ...
                    @empty
                        <div class="text-center py-5">
                            ...
                            <p class="text-muted mb-0">Pengajuan peminjaman dan pengembalian dari pegawai akan muncul di halaman ini.</p>
                        </div>
                    @endforelse
